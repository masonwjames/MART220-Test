...
function preload() {
  // Load images
  ramenImg = loadImage('images/ramen.png');
  chopImg = loadImage('images/chopsticks.png');
  aiImg = loadImage('images/ai_ramen.png');
  
  // Load fonts
  titleFont = loadFont('assets/Lobster.ttf');
  nameFont = loadFont('assets/Lobster.ttf');
}
...
function setup() {
  ...
  function draw() {
    ...
    drawMovingImage();  // Call the image moving function
    drawTitleAndName();  // Call the title and name drawing function
}
  ...
  function drawTitleAndName() {
    ...
    textSize(24);
    textAlign(LEFT, TOP);
    text("RAMEN BOWL WITH ANIMATION", 10, 10);
    
    // Name text with custom font
    textFont(nameFont);
    textSize(12);
...
function drawMovingImage() {
  fill(150, 150, 255);
  rect(600, imageY, 80, 80, 10);

  // Move image with timer
  imageY += imageDirection * 1;
  if (timer % 100 === 0) {
    imageDirection *= -1; // Change direction every 100 frames
  }
}

function keyPressed() {
  // Use arrow keys to move pork slices
...
function mousePressed() {
  // Check for mouse interactions with seaweed
...
function mouseDragged() {
  // Drag seaweed based on mouse position
  if (draggingSeaweed1) {
